...
export default function SingleCallPage() {
    ...
    const handleDial = async (e) => {
        ...
        try {
            ...
            if (ttsScript) formData.append('tts_script', ttsScript);
            if (ttsIntroFile) formData.append('tts_intro_file', ttsIntroFile);
            if (ttsOutroFile) formData.append('tts_outro_file', ttsOutroFile);
                ...
                <div className="bg-white shadow sm:rounded-lg px-4 py-5 sm:p-6">
                    ...
                    <form onSubmit={handleDial} className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            {(mode === 'tts_template' || mode === 'audio_only') && (
                            <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 text-xs uppercase tracking-wider">
                                        TTS Intro Clip <span className="text-gray-400 font-normal lowercase">(optional)</span>
                                    </label>
                                    <input type="file" accept=".wav,.gsm,.mp3,.ulaw,.alaw" onChange={e => setTtsIntroFile(e.target.files?.[0] || null)} className="mt-1 block w-full text-xs text-gray-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 text-xs uppercase tracking-wider">
                                        TTS Outro Clip <span className="text-gray-400 font-normal lowercase">(optional)</span>
                                    </label>
                                    <input type="file" accept=".wav,.gsm,.mp3,.ulaw,.alaw" onChange={e => setTtsOutroFile(e.target.files?.[0] || null)} className="mt-1 block w-full text-xs text-gray-500" />
                                </div>
                            </div>
                        )}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
