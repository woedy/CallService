...
export default function TemplateManagerPage() {
  ...
  const [tplIntroAudio, setTplIntroAudio] = useState(null);
  const [tplOutroAudio, setTplOutroAudio] = useState(null);
  ...
  const handleEditCategory = async (cat) => {
    ...
    if (!name) return;
    const description = window.prompt('Category description', cat.description || '');
    try {
      await api.patch(`/template-categories/${cat.id}/`, {
        name,
  ...
  const handleCreateTemplate = async (e) => {
    ...
    if (tplPromptText) formData.append('prompt_text', tplPromptText);
    formData.append('greeting_audio', tplAudio);
    if (tplIntroAudio) formData.append('tts_intro_audio', tplIntroAudio);
    if (tplOutroAudio) formData.append('tts_outro_audio', tplOutroAudio);
    ...
    try {
      ...
      setTplPromptText('');
      setTplAudio(null);
      setTplIntroAudio(null);
      setTplOutroAudio(null);
      setTplActive(true);
  ...
  const handleEditTemplate = async (tpl) => {
    ...
    if (!name) return;
    const promptText = window.prompt('Prompt text (for TTS)', tpl.prompt_text || '');
    try {
      await api.patch(`/audio-templates/${tpl.id}/`, {
        name,
            ...
            {categories.map((cat) => (
              <div key={cat.id} className="border rounded p-3 flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <button onClick={() => handleEditCategory(cat)} className="text-xs text-indigo-600 hover:underline">Edit</button>
        ...
        <div className="bg-white shadow sm:rounded-lg px-4 py-5 sm:p-6">
          ...
          <form onSubmit={handleCreateTemplate} className="space-y-3">
            ...
            <textarea
              ...
              placeholder="Question text for TTS personalization (e.g. How many cars do you own?)"
            ...
            <input type="file" accept=".wav,.gsm,.mp3,.ulaw,.alaw" onChange={(e) => setTplIntroAudio(e.target.files?.[0] || null)} className="w-full text-xs" />
            <input type="file" accept=".wav,.gsm,.mp3,.ulaw,.alaw" onChange={(e) => setTplOutroAudio(e.target.files?.[0] || null)} className="w-full text-xs" />
            ...
            <div key={tpl.id} className="border rounded p-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {tpl.prompt_text && <p className="text-xs text-gray-500">Prompt: {tpl.prompt_text}</p>}
                  <p className="text-xs text-gray-500">Status: {tpl.is_active ? 'Active' : 'Inactive'}</p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => handleEditTemplate(tpl)} className="text-xs text-indigo-600 hover:underline">Edit</button>
              ...
              {tpl.tts_intro_audio_url && (
                <audio controls className="mt-2 w-full" preload="none">
                  <source src={tpl.tts_intro_audio_url} />
                </audio>
              )}
              {tpl.tts_outro_audio_url && (
                <audio controls className="mt-2 w-full" preload="none">
                  <source src={tpl.tts_outro_audio_url} />
                </audio>
              )}
            </div>
          ))}
